...
class Package extends \Magento\Catalog\Model\Product\Type\AbstractType
{
     ...
     *
     * @var \Beckin\PackageProduct\Model\ResourceModel\Product\Link
     */
     ...
     * @param \Beckin\PackageProduct\Model\ResourceModel\Product\Link $catalogProductLink
    ...
    public function __construct(
        ...
        \Beckin\PackageProduct\Model\ResourceModel\Product\Link $catalogProductLink,
    ...
    public function getRelationInfo()
    {
        ...
        $info->setTable(
            ...
            'link_type_id=' . \Beckin\PackageProduct\Model\ResourceModel\Product\Link::LINK_TYPE_PACKAGE
        );
        return $info;
    }
    ...
    public function getChildrenIds($parentId, $required = true)
    {
        return $this->productLinks->getChildrenIds(
            $parentId,
            \Beckin\PackageProduct\Model\ResourceModel\Product\Link::LINK_TYPE_PACKAGE
        );
    }
    ...
    {
        $productsInfo = $buyRequest->getSuperPackage() ?: [];
     ...
     * Perform standard preparation process and add logic specific to Package product type.
     *
     * @param \Magento\Framework\DataObject $buyRequest
     * @param \Magento\Catalog\Model\Product $product
    ...
    /**
     * Prepare selected qty for packaged product's options
     *
     * @param  \Magento\Catalog\Model\Product $product
     * @param  \Magento\Framework\DataObject $buyRequest
     * @return array
    ...
    {
        $superPackage = $buyRequest->getSuperPackage();
        $superPackage = is_array($superPackage) ? array_filter($superPackage, 'intval') : [];

        $options = ['super_package' => $superPackage];
    ...
    /**
     * Delete data specific for Package product type
     *
     * @param \Magento\Catalog\Model\Product $product
     * @return void
     * @SuppressWarnings(PHPMD.UnusedFormalParameter)
    ...
    public function beforeSave($product)
    {
        ...
        if ($product->hasData('product_options') && !empty($product->getData('product_options'))) {
            throw new \Exception('Custom options for package product type are not supported');
        }
