...
use Beckin\PackageProduct\Model\Product\Type\Package as PackageProductType;
...
/**
 * Data provider for Package products
 * @SuppressWarnings(PHPMD.CouplingBetweenObjects)
 */
class Package extends AbstractModifier
{
    const PACKAGE_PACKAGE = 'package';
    const PACKAGE_CONTENT = 'content';
    const DATA_SCOPE_PACKAGE = 'package';
    const SORT_ORDER = 20;
    ...
    protected $uiComponentsConfig = [
        'button_set' => 'package_products_button_set',
...
echo $linkItem->getExtensionAttributes()->getAssignedprice();
        return [
            ...
            'assignedprice' => $linkItem->getExtensionAttributes()->getAssignedprice(),
    ...
    {
        if ($this->locator->getProduct()->getTypeId() === PackageProductType::TYPE_CODE) {
            $meta = array_replace_recursive(
                $meta,
                [
                    static::PACKAGE_PACKAGE => [
                        'children' => $this->getChildren(),
                        'arguments' => [
                            'data' => [
                                'config' => [
                                    'label' => __('Packaged Associated Products'),
                                    'collapsible' => true,
                                    'opened' => true,
                                    'componentType' => Form\Fieldset::NAME,
                                    'sortOrder' => $this->getNextGroupSortOrder(
                                        $meta,
                                        static::PACKAGE_CONTENT,
                                        static::SORT_ORDER
                                    ),
                                ],
                            ],
    ...
    {
        if ($packageCode = $this->getGroupCodeByField($meta, 'container_' . self::$codeQuantityAndStockStatus)) {
            $parentChildren = &$meta[$packageCode]['children'];
        ...
        if ($packageCode = $this->getGroupCodeByField($meta, self::$codeQtyContainer)) {
            $parentChildren = &$meta[$packageCode]['children'];
                ...
                'data' => [
                    'config' => [
                        ...
                        'provider' =>
                            ...
                            . '_data_source',
                        'options' => [
                            'title' => __('Add Products to Package'),
                            'buttons' => [
                                [
                                    'text' => __('Cancel'),
                                    'actions' => ['closeModal']
            ...
            'arguments' => [
                'data' => [
                    'config' => [
                        ...
                        'label' => false,
                        'content' => __(
                            'A Package product is made up of multiple, standalone products that are presented '
                            . 'as a Package. You can Package standalone products into one main Packaged product.'
                        ),
                ...
                'arguments' => [
                    'data' => [
                        'config' => [
                            ...
                            'formElement' => Form\Element\Input::NAME,
                            'componentType' => Form\Field::NAME,
                            'dataScope' => 'assignedprice',
                            'label' => __('Assignedprice'),
                            'fit' => true,
                            'sortOrder' => 85,
                            'validation' => [
                                'validate-number' => true,
